...
It's a little more complex this time because we have read off two strength values and pass them to the `update_RW` function.^[In fact, we could make this code a lot simpler if we'd recorded the strengths as a **matrix** rather than two vectors, but since I haven't introduce matrices yet I've left it in this slightly more clunky form with vectors] Hopefully it is clear what this code is doing.
