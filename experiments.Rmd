...
That intuition is correct, but we'll need to modify our approach a little bit. Earlier on when we discussed [data visualisation](./visualising-data.html) I didn't really talk much about the capabilities of different *graphics devices* because it didn't matter much. In this context it starts to matter whether we draw the plot in the RStudio plot pane or somewhere else, because there are different graphics devices associated with each one, and only some of them are responsive to user input. Clearly, in order to design a behavioural experiment we will need to use a device that supports interactivity, and that means the RStudio graphics device is no good to us: it's designed for the user to draw plots, not for interaction. On a Windows system you can use the default `windows()` graphics device, and on a Mac you can use `X11()`.^[Note that newer Macs don't ship with X11 by default anymore: you can download [XQuartz](https://www.xquartz.org/) to fix this]

The [getGraphicsEvent](https://www.rdocumentation.org/packages/grDevices/versions/3.5.1/topics/getGraphicsEvent) function solves our problem. An example of this is included in the [correlationtask.R](./scripts/correlationtask.R) script.
