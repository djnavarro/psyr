...
At this point our workspace contains one variable, a tibble called `dat`. This tibble is acting as a container for 3 variables, each of which has 10 observations. Conceptually this is very tidy, but now we have a practical question ... how do we get the information out again? There are two qualitatively different ways to do this,^[Technically this is a lie: there are many more ways to do this, but let's not make this any more difficult than it needs to be, yeah?] reflecting two different ways to think about your data:
...
To start with, let's think of `dat` as a list of variables. This was the perspective we took when contructing `dat` in the first place: we took three different vectors (`region`, `support`, `RT`) and bound them together into a data frame. From this perspective, what we want is an operator that will extract one of those variables for us. This is the role plaed by `$`. If I want to refer to the `region` variable contained *within* the `dat` tibble, I would use this command:

```{r}
dat$region
```

As you can see, the output looks exactly the same as it did for the original variable: `dat$region` *is a vector* (an unordered factor in this case), and we can refer to an element of that vector in the same way we normally would:

```{r}
dat$region[1]
```

Conceptually, the metaphor here is `dataframe$variable[value]`. The table below illustrates this by showing what type of output you get with different commands:


```{r,echo=FALSE}
df <- tibble(
  "command" = c("dat", "dat$RT", "dat$RT[1]"),
  "data frame" = c("data frame", "vector", "element"),
  "tibble" = c("tibble", "vector", "element")
)
knitr::kable(df)
rm(df)
```  
...
If we omit values for the rows (or columms) *while keeping the comma* then R will assume you want all rows (or colummns). So this returns every row in `dat` but only the first two columns:
```{r}
dat[, 1:2]
```
...
Annoyingly, this is not the case for a pure data frame. For a pure data frame, any time it is possible for R to treat the result as something else, it does: if `dat` were a pure data frame, the results would be different in some cases. This has caused my students (and myself) no end of frustration over the years because everyone forgets about this particular property of data frames and stuff breaks. In the original version of these notes published in *Learning Statistics with R* I had a length explanation of this behaviour. Nowadays I just encourage people to use tibbles instead. For what it's worth, if you are working with pure data frames, here's a summary of what to expect:

```{r,echo=FALSE}
df <- tibble(
  "command" = c("dat[1,1]", "dat[1,]", "dat[,1]", "dat[2:3,]","dat[,2:3]"),
  "data frame" = c("element", "data frame", "vector", "data frame", "data frame"),
  "tibble" = c("tibble", "tibble", "tibble", "tibble", "tibble")
)
knitr::kable(df)
rm(df)
```
...
The final panel highlights how lists can become more complicated.^[Speaking of complications... Under the hood, data frames and tibbles are secretly lists, so you can use the list indexing methods for them and so, for example, `dat[[3]]` is the same as `dat$RT`. Probably best not to worry too much about that detail right now `r emo::ji("grimace")`] Lists are just fancy containers, and there's no reason why lists can't contain other lists. In the pepper shaker scenario, if each sachet is itself a list, we would need to type `x[[1]][[1]]` to extract a grain of pepper! `r emo::ji("pepper")`
