...
Note that `dat` is a completely self-contained variable. Once you’ve created it, it no longer depends on the original variables from which it was constructed. That is, if we make changes to the original `RT` variable, it will not lead to any changes to the `RT` data stored in `dat`. So, for the sake of my sanity, let's remove all variables from the workspace *except* for the `dat` data frame:
