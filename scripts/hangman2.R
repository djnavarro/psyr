...
hangman <- function(word, errors) {
  ...
  # record everything
  dat <- c(word, guesses, errors, result)
...
words <- c("computational", "cognitive", "science")
errors <- c(4,4,4)
nwords <- length(words)
d <- matrix(NA,nwords,4)
colnames(d) <- c("word","guesses","errors","result")

for(w in 1:nwords) {
  d[w,] <- hangman(words[w], errors[w])
  
  cat("\n\n outcome: you", d[w,"result"], "\n\n")
