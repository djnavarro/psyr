...
#     that it doesn't show up when the document displays
...
# do the assignment in an environment that will be hidden
# when the page outputs the workspace
lsr_env <- new.env()
...
knitr::knit_hooks$set(output = function(x, options) {
  ...
  if (length(lines) == 1) { # first n lines
    ...
    x <- c(
      if (abs(lines[1]) > 1 | lines[1] < 0) more else NULL,
      x[lines],
