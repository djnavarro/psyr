...
```{r sampling_without_replacement, fig.show='animate', interval=1, cache=TRUE, echo=FALSE, message=FALSE,fig.height=4, fig.width=8}
library(scico)
for(i in 1:10) {
  n <- 20
  s <- 10
  ...
  pic <- d %>% ggplot(aes(x=x,y=y,colour=col)) +
...
```{r sampling_with_replacement, fig.show='animate', interval=1, cache=TRUE, echo=FALSE, message=FALSE,fig.height=4, fig.width=8}
library(scico)
for(i in 1:10) {
  n <- 20
  s <- 10
